...
<div class="flex flex-col items-center mt-8 mb-4">
    ...
    <div class="w-full overflow-x-auto shadow-md sm:rounded-lg mb-2 mt-4">
    <table class="w-full text-left rtl:text-right text-black dark:text-gray-400 border-collapse">
    ...
    <a href="{{ route('activer', ['id' => $user->id]) }}" onclick="return confirm('Voulez-vous vraiment désactiver cet utilisateur ?');" class="text-green-600 dark:text-green-500 hover:underline transition duration-300 ease-in-out">Activer</a>
@else
    <a href="{{ route('desactiver', ['id' => $user->id]) }}" onclick="return confirm('Voulez-vous vraiment activer cet utilisateur ?');" class="text-red-600 dark:text-red-500 hover:underline transition duration-300 ease-in-out">Désactiver</a>
@endif


                |
                <a href="{{ route('modifier') }}?id={{ $user->id }}" onclick="return confirm('Voulez-vous vraiment modifier cet utilisateur ?');" class="text-green-600 dark:text-green-500 hover:underline transition duration-300 ease-in-out">Modifier</a>
            </td>
        </tr>
    @endif
@endforeach
